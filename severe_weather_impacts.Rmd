...
```{r message=FALSE, echo=FALSE, results=FALSE,ref.label=""}

library(dplyr)
number_event_types_without_harm <- nrow(data %>% distinct(EVTYPE) %>% filter(FATALITIES + INJURIES <= 0)  %>% select(EVTYPE))
```

Population harm data has `r number_event_types_without_harm` levels with no injuries or fatalities.

```{r message=FALSE}

library(dplyr)
nrow(data %>% distinct(EVTYPE) %>% filter(FATALITIES + INJURIES <= 0)  %>% select(EVTYPE))
```

These can be removed from our analysis of population harm as they account for no harm.

```{r}
events_with_nonzero_harm <- data %>% distinct(EVTYPE) %>% filter(FATALITIES + INJURIES > 0)  %>% select(EVTYPE)
```
